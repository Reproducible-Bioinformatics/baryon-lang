...
func (t *RTranspiler) writeDockerHelpers() {
	...
	t.WriteLine("#' in the system's `PATH`. It returns a list with a boolean indicating if Docker")
	t.WriteLine("#' is found, and the path to the Docker executable if it is available.")
	t.WriteLine("#'")
	t.WriteLine("#' @return")
	t.WriteLine("#' A list with two elements:")
	t.WriteLine("#' \\item{found}{A logical value: `TRUE` if Docker is available in the system's")
	t.WriteLine("#' `PATH`, `FALSE` otherwise.}")
	t.WriteLine("#' \\item{path}{A character string containing the full path to the Docker")
	t.WriteLine("#' executable if found,  or an empty string if not found.}")
	t.WriteLine("#'")
	t.WriteLine("#' @examples")
	t.WriteLine("#' result <- has_docker()")
	t.WriteLine("#' if (result$found) {")
	t.WriteLine("#'   cat(\"Docker is available at:\", result$path, \"\\n\")")
	t.WriteLine("#' } else {")
	t.WriteLine("#'   cat(\"Docker is not available.\\n\")")
	t.WriteLine("#' }")
	t.WriteLine("#'")
	t.WriteLine("#' @export")
	t.WriteLine("has_docker <- function() {")
	t.WriteLine("  path <- Sys.which(\"docker\")")
	t.WriteLine("  return(list(found = nzchar(path), path = path, fn = run_in_docker))")
	t.WriteLine("}")
	t.WriteLine("#' Check if the script is running in Docker.")
	t.WriteLine("#'")
	t.WriteLine("#' @returns A truthy value indicating the state.")
	t.WriteLine("#' @export")
	t.WriteLine("is_running_in_docker <- function() {")
	t.WriteLine("  dockerenv_exists <- file.exists(\"/.dockerenv\")")
	t.WriteLine("  cgroup_exists <- file.exists(\"/proc/1/cgroup\")")
	t.WriteLine("  in_container_runtime <- FALSE")
	t.WriteLine("  if (cgroup_exists) {")
	t.WriteLine("    in_container_runtime <- any(")
	t.WriteLine("      grepl(\"docker\", readLines(\"/proc/1/cgroup\", warn = FALSE))")
	t.WriteLine("    )")
	t.WriteLine("  }")
	...
	t.WriteLine("    )")
	t.WriteLine("    base_command <- paste(base_command, \"-v\", paste(")
	t.WriteLine("      volume[1],")
	t.WriteLine("      volume[2],")
	t.WriteLine("      sep = \":\"")
	t.WriteLine("    ))")
	t.WriteLine("  }")
